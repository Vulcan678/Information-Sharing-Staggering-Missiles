function [t,xm,ym,xt,yt,nc,miu,miss,x_true,x_est,p_est] = Sharing_SingleMode_EstimationCheck(xm0,ym0,tau,am_max,vm,k,xt0,yt0,theta,at_max,T_sw,vt,gamma_t0,xr,yr,dt,Dt,Nmis,sigmas,delta_m_sig,noise_int,noise_mes,pi_mat,Q_n,DB,h,S_miss1_mean,S_miss1_SD,Q_n_red)
% Inputs: 
...
while t(i)<2*max(max(rho))/(vt+vm)
            ...
            if rho_check(ind)>rho_check(ind-1)
                ...
                if count(mis)>3
                    ...
                    if miss_est>5
                        ...
                        %%% switch time + continue the simulation based on
                        %%% the estimated mode with reduced value in
                        %%% Q_n(4,4)
                        [T_sw_est,i_sw_est,mode_before,mode_after] = switch_time_est(miss_est,DB,h,Dt,miu,S_miss1_mean,S_miss1_SD);
                        runKF=1;
                        Q_n(4,4)=Q_n(4,4)/Q_n_red;
                    end
                    break
                end
            else
    ...
    if runKF==0    
        ...
        %%% in case the first missile missed we continue the simulation
        %%% only with EKF and not IMM
        mis=2;
        j=mode_after;
        
        %%% estimation
        rho_mix=x_n1_n1(1,j,mis);
        lambda_mix=x_n1_n1(2,j,mis);
        gamma_t_mix=x_n1_n1(3,j,mis);
        at_mix=x_n1_n1(4,j,mis);
        ...
        rho_now_est(1:length(res_est_temp(:,3)),mis)=res_est_temp(:,3);
        ...
        %%% if the differance of one of the estimated measurments and the mesuarment is larger than 4*SD we ignore the measurments
        if abs(y_n_mes1(i,mis)-y_n_n11(i,j,mis))>4*sqrt(S1(j))  
            x_n_n(:,:,mis)=x_n_n1(:,:,mis);
            p_n_n(:,:,:,mis)=p_n_n1(:,:,:,mis);
        end
        
        x_true(:,i,mis)=[rho(i,mis);lambda(i,mis);gamma_t(i);at(i)];
        x_est(:,i,mis)=x_n_n(:,j,mis);
        p_est(:,:,i,mis)=p_n_n(:,:,j,mis);
