function [t,xm,ym,xt,yt,nc,miu,miss,x_true,x_est,p_est] = Sharing_IMM_Sim_EstimationCheck(xm0,ym0,tau,am_max,vm,k,xt0,yt0,theta,at_max,T_sw,vt,gamma_t0,xr,yr,dt,Dt,Nmis,sigmas,delta_m_sig,noise_int,noise_mes,pi_mat,Q_n,DB,h,S_miss1_mean,S_miss1_SD,Q_n_red,TPM_new,TPM_new_sim)
% Inputs: 
...
% TPM_new_sim - the transition matrix for the inner simulation
% Outputs: 
...
while t(i)<2*max(max(rho))/(vt+vm)
    ...
    if runSim==1
        ...
        pi_mat_sim=TPM_new_sim;
        
        % initialize
        if mode_before==1
            miu_sim(:,1,1)=[1-1e-3;1e-3];
            miu_sim(:,1,2)=[1-1e-3;1e-3];
        else
            miu_sim(:,1,1)=[1e-3;1-1e-3];
            miu_sim(:,1,2)=[1e-3;1-1e-3];
        end
        
        x_n_n_sim(:,1,1)=x_est(:,1,1);
        x_n_n_sim(:,2,1)=x_est(:,1,1);
        x_n_n_sim(:,1,2)=x_est(:,1,2);
        x_n_n_sim(:,2,2)=x_est(:,1,2);
        x_est_sim(:,1,:)=x_est(:,1,:);
        
        p_n_n_sim(:,:,1,1)=p_est(:,:,1,1);
        p_n_n_sim(:,:,2,1)=p_est(:,:,1,1);
        p_n_n_sim(:,:,1,2)=p_est(:,:,1,2);
        p_n_n_sim(:,:,2,2)=p_est(:,:,1,2);
        p_est_sim(:,:,1,:)=p_est(:,:,1,:);
        ...
        for i_sim=2:i-1
            for mis=1:Nmis  
                ...
                % estimation
                miu_ij_sim(:,:,mis)=[pi_mat_sim(1,1)*miu_sim(1,i_sim-1,mis),pi_mat_sim(1,2)*miu_sim(1,i_sim-1,mis);
                    pi_mat_sim(2,1)*miu_sim(2,i_sim-1,mis),pi_mat_sim(2,2)*miu_sim(2,i_sim-1,mis)];
                miu_j1_sum_sim(mis)=sum(miu_ij_sim(:,1,mis));
                miu_j2_sum_sim(mis)=sum(miu_ij_sim(:,2,mis));
                miu_ij_sim(:,:,mis)=[miu_ij_sim(:,1,mis)/miu_j1_sum_sim(mis),miu_ij_sim(:,2,mis)/miu_j2_sum_sim(mis)];
                
                %%% estimation for each mode
                for j=1:2
                    x_n1_n1_0=miu_ij_sim(1,j,mis)*x_n1_n1_sim(:,1,mis)+miu_ij_sim(2,j,mis)*x_n1_n1_sim(:,2,mis);
                    p_n1_n1_0=miu_ij_sim(1,j,mis)*(p_n1_n1_sim(:,:,1,mis)+(x_n1_n1_sim(:,1,mis)-x_n1_n1_0)*transpose(x_n1_n1_sim(:,1,mis)-x_n1_n1_0))+miu_ij_sim(2,j,mis)*(p_n1_n1_sim(:,:,2,mis)+(x_n1_n1_sim(:,2,mis)-x_n1_n1_0)*transpose(x_n1_n1_sim(:,2,mis)-x_n1_n1_0));
                    ...
                    help_est=[vm,vt,nc(i_sim-1,mis),2*(j-1.5)*at_max,tau,theta];
                    ...
                    if dt_sim<5e-4        
                        ...
                        for ind=1:length(dt_ar)   
                            ...
                            p_n_n1_sim(:,:,j,mis)=phi_n*p_n_n1_sim(:,:,j,mis)*transpose(phi_n)+Q_n_sim;
                        end
                    else
                        F=[0,v_lambda_mix,vt*sin(delta_t_mix),0;
                            -v_lambda_mix/(rho_mix^2),-v_rho_mix/rho_mix,vt*cos(delta_t_mix)/rho_mix,0;
                        ...
                        p_n_n1_sim(:,:,j,mis)=phi_n*p_n1_n1_0*transpose(phi_n)+Q_n_sim;
                    end 
                    x_n_n1_sim(:,j,mis)=[rho_j(j,mis);lambda_j(j,mis);gamma_t_j(j,mis);at_j(j,mis)];
                    ...
                    y_n_n1_sim(:,i_sim,j,mis)=[gamma_m(i_sim,1)-atan2(ym(i_sim,mis)-ym(i_sim,1)+rho_j(j,mis)*sin(lambda_j(j,mis)),xm(i_sim,mis)-xm(i_sim,1)+rho_j(j,mis)*cos(lambda_j(j,mis)));
                        gamma_m(i_sim,2)-atan2(ym(i_sim,mis)-ym(i_sim,2)+rho_j(j,mis)*sin(lambda_j(j,mis)),xm(i_sim,mis)-xm(i_sim,2)+rho_j(j,mis)*cos(lambda_j(j,mis)))];

                    S(:,:,j)=H_n(:,:,j,mis)*p_n_n1_sim(:,:,j,mis)*transpose(H_n(:,:,j,mis))+R_n;
                    ...
                    if abs((S(1,2,j)-S(2,1,j))/S(1,2,j))<1e-3
                        ...
                        Lambda(j)=(1/(2*pi*sqrt(det_S)))*exp(-((((y_n_mes(1,i_sim)-y_n_n1_sim(1,i_sim,j,mis))/sig_x)^2)+(((y_n_mes(2,i_sim)-y_n_n1_sim(2,i_sim,j,mis))/sig_y)^2)-((2*rho_cor*(y_n_mes(1,i_sim)-y_n_n1_sim(1,i_sim,j,mis))*(y_n_mes(2,i_sim)-y_n_n1_sim(2,i_sim,j,mis)))/(sig_x*sig_y)))/(2*(1-rho_cor^2)));
                        K=p_n_n1_sim(:,:,j,mis)*transpose(H_n(:,:,j,mis))*inv_S;
                    else
                        %%% mode conditioned likelihood
                        Lambda(j)=(1/(2*pi*sqrt(det(S(:,:,j)))))*exp(-0.5*transpose(y_n_mes(:,i_sim)-y_n_n1_sim(:,i_sim,j,mis))*inv(S(:,:,j))*(y_n_mes(:,i_sim)-y_n_n1_sim(:,i_sim,j,mis)));
                        K=p_n_n1_sim(:,:,j,mis)*transpose(H_n(:,:,j,mis))*inv(S(:,:,j));
                    end

                    x_n_n_sim(:,j,mis)=x_n_n1_sim(:,j,mis)+K*(y_n_mes(:,i_sim)-y_n_n1_sim(:,i_sim,j,mis));
                    p_n_n_sim(:,:,j,mis)=(eye(4)-K*H_n(:,:,j,mis))*p_n_n1_sim(:,:,j,mis)*transpose(eye(4)-K*H_n(:,:,j,mis))+K*R_n*transpose(K);
                     
                end
                   
                %%% if the differance of one of the estimated measurments and the mesuarment is larger than 4*SD we ignore the measurments
                if abs(y_n_mes(1,i_sim)-y_n_n1_sim(1,i_sim,1,mis))>4*sqrt(S(1,1,1)) || abs(y_n_mes(2,i_sim)-y_n_n1_sim(2,i_sim,1,mis))>4*sqrt(S(2,2,1)) || abs(y_n_mes(1,i_sim)-y_n_n1_sim(1,i_sim,2,mis))>4*sqrt(S(1,1,2)) || abs(y_n_mes(2,i_sim)-y_n_n1_sim(2,i_sim,2,mis))>4*sqrt(S(2,2,2))
                    Lambda=0*Lambda;            
                ...
                miu_sum=Lambda(1)*miu_j1_sum_sim(mis)+Lambda(2)*miu_j2_sum_sim(mis);

                if isnan(miu_sum)==1 || isinf(miu_sum)==1 || isreal(miu_sum)==0 || miu_sum<=0
                    miu_sim(:,i_sim,mis)=miu_sim(:,i_sim-1,mis);
                else
                    %%% limitiations on the value of miu
                    miu1_temp=Lambda(1)*miu_j1_sum_sim(mis)/miu_sum;
                    miu_min=1e-3;
                    ...
                    miu_sim(1,i_sim,mis)=miu1_temp;
                    miu_sim(2,i_sim,mis)=1-miu1_temp;
                end
                
                 % chose the targets manuver according to the estimated mode and T_sw
                if abs(i_sim-i_sw_est)<0.5 
                    if mode_after==1
                        miu_sim(:,i_sim,mis)=[1-1e-1;1e-1];
                    else
                        miu_sim(:,i_sim,mis)=[1e-1;1-1e-1];
                    end
                end

                %%% mix the results of the estimation
                x_est_sim(:,i_sim,mis)=x_n_n_sim(:,1,mis)*miu_sim(1,i_sim,mis)+x_n_n_sim(:,2,mis)*miu_sim(2,i_sim,mis);
                p_est_sim(:,:,i_sim,mis)=miu_sim(1,i_sim,mis)*(p_n_n_sim(:,:,1,mis)+(x_n_n_sim(:,1,mis)-x_est_sim(:,i_sim,mis))*transpose((x_n_n_sim(:,1,mis)-x_est_sim(:,i_sim,mis))))+miu_sim(2,i_sim,mis)*(p_n_n_sim(:,:,2,mis)+(x_n_n_sim(:,2,mis)-x_est_sim(:,i_sim,mis))*transpose((x_n_n_sim(:,2,mis)-x_est_sim(:,i_sim,mis))));
